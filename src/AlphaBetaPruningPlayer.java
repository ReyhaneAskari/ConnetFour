...
//        winner2 = 0;
    ...
    private int evaluateFirstPlayerMove(int depth, int maxDepth, int col , int alpha , int betha) {
        ...
        for (int c = 0; c < 7; c++) {
            if (board.isAvailable(c)) {
                ...
                int value = evaluateSecondPlayerMove(depth + 1, maxDepth, c , Integer.MIN_VALUE + 1 , Integer.MAX_VALUE -1);
                board.unset(c);
                if (value > max) {
                    max = value;
                    if (depth == 0) {
    ...
    private int evaluateSecondPlayerMove(int depth, int maxDepth, int col , int alpha , int beta) { 
        ...
        for (int c = 0; c < 7; c++) {
            if (board.isAvailable(c)) {
                ...
                int value = evaluateFirstPlayerMove(depth + 1, maxDepth, c ,Integer.MIN_VALUE + 1 , Integer.MAX_VALUE -1);
                board.unset(c);
                if (value < min) {
                    min = value;
                    if (depth == 0) {
...
//        winner2 = 0;
		win = lose = false;
            ...
            evaluateFirstPlayerMove(0, 1, -1 ,Integer.MIN_VALUE +1 ,Integer.MAX_VALUE -1 );
            if (win) {
//            	System.out.println("decided as win");
                return outcol;
            }
            win = lose = false;
            evaluateSecondPlayerMove(0, 1, -1 , Integer.MIN_VALUE +1 , Integer.MAX_VALUE -1);
            if (lose) {
//            	System.out.println("decided as lost");
                return outcol;
            }
            evaluateFirstPlayerMove(0, maxDepth, -1 ,Integer.MIN_VALUE + 1 , Integer.MAX_VALUE -1 );
